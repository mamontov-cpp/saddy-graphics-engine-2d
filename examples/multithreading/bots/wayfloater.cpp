...
bots::WayFloater::~WayFloater() = default;
...
void bots::WayFloater::perform(Game* game, game::Actor* actor)
{
    if (!m_way)
    {
        if (game)
        {
            m_way = game->rendererForMainThread()->database("gamescreen")->objectByName<sad::Way>(m_way_name);
        }
    }
    if (m_way && game)
    {
        const double step = 1000.0 / (game->rendererForMainThread()->fps());
