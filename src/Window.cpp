...
BOOL CreateWindowGL (GL_Window* window)									// This Code Creates Our OpenGL Window
{
	...
	window->hWnd = CreateWindowEx (windowExtendedStyle,					// Extended Style
		(LPCSTR)window->init.application->className,	// Class Name
		(LPCSTR)window->init.title,					// Window Title
...
{																		// TRUE If Successful
	...
	windowClass.lpszClassName	= (LPCSTR)application->className;				// Sets The Applications Classname
	if (RegisterClassEx (&windowClass) == 0)							// Did Registering The Class Fail?
	{
		// NOTE: Failure, Should Never Happen
		MessageBox (HWND_DESKTOP, LPCSTR("RegisterClassEx Failed!"), LPCSTR("Error"), MB_OK | MB_ICONEXCLAMATION);
...
int WINAPI WinMain (HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	application.className = "SadEngine";									// Application Class Name
	...
	window.init.title			= (char *)MainSettings.TITLE.data();			// Window Title
	window.init.width			= MainSettings.SCREEN_WIDTH;			// Window Width
	window.init.height			= MainSettings.SCREEN_HEIGHT;			// Window Height
	window.init.bitsPerPixel	= MainSettings.BPP;						// Bits Per Pixel
	window.init.isFullScreen	= MainSettings.FULLSCREEN;				// Fullscreen? (Set To TRUE)
	...
	{
		// Failure
		MessageBox (HWND_DESKTOP, LPCSTR("Error Registering Window Class!"), LPCSTR("Error"), MB_OK | MB_ICONEXCLAMATION);
	...
	sadInitTimeOperation();
	SadTimer myTimer;
	myTimer.sadStartTiming();				// Get The Tick Count
				...
				while (isMessagePumpActive == TRUE)						// While The Message Pump Is Active
				{
					...
					else												// If There Are No Messages
					{
						...
						else											// If Window Is Visible
						{
							...
							Update (myTimer.sadTimeElapsed());	// Update The Counter
							myTimer.sadStartTiming();				// Get The Tick Count
	...
	UnregisterClass (LPCSTR(application.className), application.hInstance);		// UnRegister Window Class
	return 0;
}																		// End Of WinMain()
